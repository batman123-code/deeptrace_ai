...
const OptionWheel = ({
  ...
  // Hardcode remPx to 16 to avoid NaN issues from getComputedStyle during hydration
  const remPx = 16;

  onChangeRef.current = onChange;
  const computedRowH = Number(fontSize) * Number(spacing) * remPx;
  
  cfgRef.current = {
    count: items.length,
    items,
    rowH: isNaN(computedRowH) ? 72 : Math.max(computedRowH, 1),
    curve: Number(curve) || 0,
    tilt: Number(tilt) || 0,
    blur: Number(blur) || 0,
    fade: Number(fade) || 0,
    minOpacity: Number(minOpacity) || 0,
    side,
    loop,
    smoothing: Number(smoothing) || 200,
    draggable,
    soundUrl,
    soundVolume: Number(soundVolume) || 0.5
  };
  ...
  const runFrame = useCallback((now: number) => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      // Prevent NaN from propagating to CSS
      if (isNaN(x)) x = 0;
      if (isNaN(y)) y = 0;
      if (isNaN(rot)) rot = 0;

      el.style.transform = `translateY(calc(-50% + ${y.toFixed(2)}px)) translateX(${x.toFixed(2)}px) rotate(${rot.toFixed(3)}deg)`;
